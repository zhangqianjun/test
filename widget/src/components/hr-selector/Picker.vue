  ...
  <div class="m-picker"
    ...
    <div class="m-picker-inner">
      ...
      <ul class="m-picker-list">
        ...
        :key="index"
        :style="{transform: 'rotateX(' + (-threshold * index) +'deg) translateZ(90px)'}">{{typeof item === 'object' ? item[labelKey] : item}}</li>
      </ul>
    </div>
  </div>
</template>
<script type="text/babel">
  ...
  export default {
    props: ['alias', 'list', 'curIdx', 'label', 'valueKey', 'labelKey'],
    data () {
      return {
        distinct: 0,
        speed: 0.3,
        ...
        threshold: 20,
        animatePause: true,
        eventType: 'touch', // 事件类型
        hasDown: false, // 是否鼠标点下
        hasMove: false, // 是否鼠标移动过
        mouseObj: { //  鼠标事件的坐标对象
          x1: 0,
          y1: 0,
          x2: 0,
        ...
        lastTimestamp: null // mousestart的时间
      }
    },
    ...
    methods: {
      mousedown(evt) {
        ...
        this.mouseObj.x2 = this.mouseObj.x1 = evt.pageX
        this.mouseObj.y2 = this.mouseObj.y1 = evt.pageY
      },
      mouseup(evt) {
        if (!this.hasDown || !this.hasMove) {
          this.hasDown = false
          this.hasMove = false
          return
        }
        this.hasDown = false
        this.hasMove = false
        this.mouseObj.x2 = evt.pageX
        this.mouseObj.y2 = evt.pageY
        evt.preventDefault()
        if (Math.abs(this.mouseObj.y1 - this.mouseObj.y2) < this.threshold * 2) {
          this.end()
        } else {
          this.animatePause = false
          this.mouseObj.spend = Date.now() - this.lastTimestamp
          this.startInertiaScroll(this.mouseObj)
        }
      },
      mousemove(evt) {
        if (!this.hasDown) {
        ...
        this.hasMove = true
        evt.preventDefault()
        var yrange = this.mouseObj.y2 - evt.pageY
        var xrange = this.mouseObj.x2 - evt.pageX
        this.mouseObj.toUp = yrange > 0
        this.mouseObj.toLeft = xrange > 0
        this.mouseObj.xrange = xrange
        this.mouseObj.yrange = yrange
        this.move(this.mouseObj)
      },
      move (res) {
        ...
        this.$emit('picker', this.curIndex)
      },
      ...
      startInertiaScroll (res) {
        ...
        var duration = Math.abs(v / 0.01)// 缓动速度
      ...
      reload () {
        ...
        this.highlight(0,this.curIndex)
        // this.highlight(0, 2)
        // this.curIndex = 2
